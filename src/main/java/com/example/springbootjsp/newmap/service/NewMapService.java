package com.example.springbootjsp.newmap.service;

import com.alibaba.fastjson.JSONObject;
import com.example.springbootjsp.map.vo.CreateVO;
import com.example.springbootjsp.map.vo.FileVO;
import com.example.springbootjsp.newmap.mapper.NewMapMapper;
import com.example.springbootjsp.newmap.vo.*;
...
public class NewMapService {
    ...
    private static final Logger log = LoggerFactory.getLogger(NewMapService.class);

    @Autowired
    NewMapMapper mapMapper;
...
//    private String filePath = "/home/centos/apache-tomcat-8.5.40/zippro/webapps/zippro-web/images/";
    ...
    // 첨부파일 삭제
    public void setFileDelete(NewFileVO paramVO){
        NewFileVO fileVO = mapMapper.getFileInfo(paramVO);
    ...
    public boolean createProductInfo(NewCreateVO createVO) {
        ...
        for(Map<String, String> file : fileInfoList) {
            NewFileVO fileVO = new NewFileVO();
        ...
        for(String option : typeList) {
            NewOptionVO optionVO = new NewOptionVO();
    ...
    public boolean modifyProductInfo(NewCreateVO createVO) {
        ...
        List<NewFileVO> deleteFileList = mapMapper.getProductFileList(createVO.getProductId());

        for(NewFileVO deleteFileVO:deleteFileList) {
            File deleteFile = new File(deleteFileVO.getFilePath()+""+deleteFileVO.getStorFileNm());
    ...
    public boolean deleteProductInfo(CreateVO createVO) {
        List<NewFileVO> fileList = mapMapper.getProductFileList(createVO.getProductId());

        for(NewFileVO fileVO:fileList) {
